...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli -regtest validateaddress "$P2SH_ADDR" | grep -o '"isvalid": true' | cut -d':' -f2 | tr -d ' ')
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo $ADDR_INFO | grep -oP 'desc": "\K[^#]*' | cut -d']' -f1 | cut -d'/' -f1)
